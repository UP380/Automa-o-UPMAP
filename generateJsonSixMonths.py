...
# Função modificada para obter todas as tarefas em uma lista criadas nos últimos 6 meses
def get_all_tasks_in_list(list_id):
    # Calcula a data de 6 meses atrás
    six_months_ago = datetime.now() - timedelta(days=3*30)  # Aproximação de 6 meses
    six_months_ago_iso = six_months_ago.strftime('%Y-%m-%dT%H:%M:%S')  # Formatação para ISO 8601

    # Adiciona parâmetros de consulta à URL
    params = {
        'created_gte': six_months_ago_iso,
    }
    
    task_response = requests.get(f'https://api.clickup.com/api/v2/list/{list_id}/task', headers=headers, params=params)
...
def get_all_folders_in_space(space_id):
    print('Só pegando mais um space!')
    ...
    print('passou aqui também, tá tudo certo')
    return all_folders
